...
{
  // 判断队列是否为空，是就返回空string_view，不是就构造一个新的string_view，该对象是 stream_.front() 的子视图，并且截取到offset_
...
void Reader::pop( uint64_t len )
{
  total_buffered_ -= len;
  total_popped_ += len;
  /* 当 len 小于队列前端元素剩余的大小时（即 len < size），只需更新 offset_ 而不是弹出元素。这避免了在每次迭代中都弹出元素，只有在必要时才进行 pop 操作
      stream_.front() 返回队列中的第一个元素的引用，在 std::string 的情况下，这个大小就是字符串的长度。*/
  while (len != 0U) {
